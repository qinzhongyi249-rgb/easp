...
func (r *ConnectorRepository) Create(connector *models.Connector) error {
	...
	query := `INSERT INTO connectors (id, tenant_id, name, type, base_url, transport_type, mcp_server_url, headers, auth_type, auth_config, credential_mode, user_token_header, user_token_prefix, user_token_required_sso, spec_url, spec_content, status, tools_count, is_builtin, locked, last_sync_at, created_at, updated_at)
			  VALUES (:id, :tenant_id, :name, :type, :base_url, :transport_type, :mcp_server_url, :headers, :auth_type, :auth_config, :credential_mode, :user_token_header, :user_token_prefix, :user_token_required_sso, :spec_url, :spec_content, :status, :tools_count, :is_builtin, :locked, :last_sync_at, :created_at, :updated_at)`
...
func (r *ConnectorRepository) Update(connector *models.Connector) error {
	...
	query := `UPDATE connectors SET name=:name, type=:type, base_url=:base_url, transport_type=:transport_type, mcp_server_url=:mcp_server_url, headers=:headers, auth_type=:auth_type, auth_config=:auth_config, credential_mode=:credential_mode, user_token_header=:user_token_header, user_token_prefix=:user_token_prefix, user_token_required_sso=:user_token_required_sso,
			  spec_url=:spec_url, spec_content=:spec_content, status=:status, tools_count=:tools_count, is_builtin=:is_builtin, locked=:locked, last_sync_at=:last_sync_at, updated_at=:updated_at WHERE id=:id`
	_, err := database.DB.NamedExec(query, connector)
	return err
}
